...
internal sealed class TestHttpServer : IDisposable
{
    ...
    private async Task ListenAsync()
    {
        while (true)
        {
            try
            {
                Task<HttpListenerContext> pendingContext;
                lock (_listenerGate)
                {
                    if (_stopping)
                        return;

                    // Windows binds the request-queue handle synchronously here. Closing the
                    // listener during that registration can throw an invalid-handle exception.
                    pendingContext = _listener.GetContextAsync();
                }

                HttpListenerContext context = await pendingContext;
